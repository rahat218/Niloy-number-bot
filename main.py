...
LANG_TEXT = {
    'bn': {
        ...
        # ✅ সমাধান ৪: নতুন প্রিমিয়াম ব্রডকাস্ট মেসেজ
        "new_number_broadcast": (
            "🎉 **সুখবর! নতুন নম্বর যোগ করা হয়েছে!** 🎉\n\n"
            "📅 তারিখ: *{date}*\n\n"
        ...
        # ✅ সমাধান ২ এবং ৩ এর জন্য নতুন টেক্সট
...
async def setup_database(app: Application):
    ...
    try:
        ...
        await app.bot.send_message(chat_id=ADMIN_USER_ID, text="✅ Bot Deployed/Restarted with all new features!")
...
# -----------------------------------------------------------------------------
# ... (এই সেকশনের সকল ফাংশন অপরিবর্তিত)
async def inactivity_strike_job(context: ContextTypes.DEFAULT_TYPE):
    job = context.job; user_id = job.data['user_id']; number_id = job.data['number_id']
...
# -----------------------------------------------------------------------------
# ... (get_main_reply_keyboard, get_number_options_keyboard, start_command, etc. অপরিবর্তিত)
    ...
    async with await get_db_conn() as aconn:
        async with aconn.cursor(row_factory=psycopg.rows.dict_row) as acur:
            ...
            if user_status and user_status['is_banned']:
                ...
                await update.message.reply_text(LANG_TEXT[lang]['status_banned'].format(hours=BAN_HOURS, unban_time=unban_time)); return
...
async def handle_my_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with await get_db_conn() as aconn:
        async with aconn.cursor(row_factory=psycopg.rows.dict_row) as acur:
            await acur.execute("SELECT strikes, is_banned, ban_until FROM users WHERE user_id = %s", (user_id,)); stats = await acur.fetchone()
            if stats:
                message = f"{LANG_TEXT[lang]['stats_header']}\n\n{LANG_TEXT[lang]['strikes']}: {stats['strikes']}/{MAX_STRIKES}\n"
                if stats['is_banned']: unban_time = stats['ban_until'].strftime('%Y-%m-%d %H:%M') if stats['ban_until'] else 'N/A'; message += LANG_TEXT[lang]['status_banned'].format(hours=BAN_HOURS, unban_time=unban_time)
...
async def handle_button_press(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id; data = query.data; lang = await get_user_lang(user_id)
    ...
    if data.startswith("get_number_"):
        ...
        async with await get_db_conn() as aconn:
            async with aconn.cursor(row_factory=psycopg.rows.dict_row) as acur:
                ...
                if user and user['last_number_success_at'] and datetime.datetime.now() < user['last_number_success_at'] + datetime.timedelta(minutes=COOLDOWN_MINUTES):
                    await query.edit_message_text(LANG_TEXT[lang]['cooldown_message'].format(minutes=COOLDOWN_MINUTES)); return
        ...
        if number_data:
            ...
            # ✅ সমাধান ১: নম্বর শেষ হয়ে গেলে অ্যাডমিনকে জানানো
            try:
                await context.bot.send_message(chat_id=ADMIN_USER_ID, text=f"⚠️ **স্টক খালি!** ⚠️\n\nব্যবহারকারীরা **{service}** সার্ভিসের জন্য নম্বর খুঁজে পাচ্ছেন না। অনুগ্রহ করে দ্রুত নতুন নম্বর যোগ করুন।")
            except Exception as e:
...
# -----------------------------------------------------------------------------
# |                      👑 অ্যাডমিন প্যানেল হ্যান্ডলার (পরিবর্তিত) 👑                      |
# -----------------------------------------------------------------------------
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_USER_ID: return
    lang = await get_user_lang(ADMIN_USER_ID)
    
    # ✅ সমাধান ৫: গাইড বাটন যোগ করা
    ...
    # মেসেজ এডিট বা নতুন মেসেজ পাঠানো
...
async def handle_admin_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ সমাধান ৫: গাইড বাটন হ্যান্ডেল করা
        ...
        await admin_panel(update, context) # অ্যাডমিন প্যানেল আবার দেখানো
...
# --- অ্যাডমিন কমান্ড হ্যান্ডলার (পরিবর্তিত) ---
...
async def add_number_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # কমা বা স্পেস দিয়ে আলাদা করা নম্বর গ্রহণ করা
    ...
    async with await get_db_conn() as aconn:
        try:
            ...
            logger.error(f"Error adding numbers: {e}"); await aconn.rollback(); await update.message.reply_text("Error adding numbers."); return
    if added_count > 0:
    ...
    else: await update.message.reply_text("No new numbers were added (duplicates).")

# ✅ সমাধান ৪: নতুন প্রিমিয়াম ব্রডকাস্ট ফাংশন
async def auto_broadcast_new_numbers(context: ContextTypes.DEFAULT_TYPE, service: str, lang: str):
    bot = context.bot
    # পুরানো ঘোষণা ডিলিট করা
    async with await get_db_conn() as aconn:
        async with aconn.cursor(row_factory=psycopg.rows.dict_row) as acur:
    ...
    for user in all_users:
        ...
        # ব্যবহারকারীর ভাষা অনুযায়ী টেক্সট ফরম্যাট করা
        text = LANG_TEXT.get(user_lang, LANG_TEXT['bn'])['new_number_broadcast'].format(
            date=current_date, 
            service=service, 
            get_number_button=GET_NUMBER_TEXT
        )
        try:
...
# --- (বাকি অ্যাডমিন কমান্ড হ্যান্ডলার অপরিবর্তিত) ---
...
# ... (বাকি সকল অ্যাডমিন এবং ইউজার ম্যানেজমেন্ট কমান্ড অপরিবর্তিত)
...
# ✅ সমাধান ২ এবং ৩: নতুন ডিলিট কমান্ড ফাংশন
...
def main() -> None:
    ...
    # --- অ্যাডমিন কমান্ড হ্যান্ডলার (নতুন কমান্ডসহ) ---
    ...
    # ✅ সমাধান ২ এবং ৩: নতুন কমান্ড হ্যান্ডলার যোগ করা
    ...
    # --- ব্যবহারকারী কমান্ড এবং বাটন হ্যান্ডলার ---
