...
# --- বহুভাষিক টেক্সট (সম্পূর্ণ এবং ত্রুটিমুক্ত) ---
LANG_TEXT = {
    'bn': {
    ...
    'en': {
        ...
        "new_number_broadcast": "🎉 **Great News! New Numbers Added!** 🎉\n\n📅 Date: *{date}*\n\nThe admin has just added new numbers for the **{service}** service. Stock is limited!\n\nClick the button below to get your number now.\n\n👇👇👇",
        ...
        "del_service_prompt": "🗑️ To delete all numbers of a service:\n`/del_service <Service>`",
        "del_service_success": "✅ Successfully deleted {count} numbers from the `{service}` service.",
        "del_all_prompt": "🔴 **WARNING!** 🔴\nAre you sure you want to delete all numbers from all services? This action cannot be undone.\n\nTo confirm, reply with: `/del_all YES`",
        "del_all_success": "✅ Successfully deleted all numbers from the database.",
        "del_all_cancelled": "❌ Deletion cancelled.",
        "admin_guide_button": "📜 Admin Guide",
        "admin_guide_header": "👑 **Admin Command Guide** 👑\n\n",
        "admin_guide_text": "**Number Management:**\n🔹 `/add <Service> <Num1> <Num2>...`\n   - Adds one or more numbers to a service.\n🔹 `/delnumber <Number>`\n   - Deletes a specific number.\n🔹 `/del_service <Service>`\n   - Deletes *all* numbers of a service.\n🔹 `/del_all`\n   - Deletes *all* numbers from the database (use with caution).\n🔹 `/reactivate <Number>`\n   - Reactivates a reported or used number.\n\n**User Management:**\n🔸 `/ban <User_ID>`\n   - Bans a user.\n🔸 `/unban <User_ID>`\n   - Unbans a user.\n\n**Communication:**\n▪️ `/broadcast <Message>`\n   - Sends a message to all users.\n▪️ `/delbroadcast`\n   - Deletes the last broadcast message.",
...
async def handle_button_press(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("get_number_"):
        ...
        if number_data:
            ...
            try:
                await context.bot.send_message(chat_id=ADMIN_USER_ID, text=f"⚠️ **Stock Alert!** ⚠️\n\nUsers are requesting numbers for **{service}**, but the stock is empty. Please add new numbers.", parse_mode='Markdown')
            except Exception as e:
