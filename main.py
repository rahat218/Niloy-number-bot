...
async def handle_get_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with await get_db_conn() as aconn:
        async with aconn.cursor() as acur:
            ...
            if not user_data: # যদি কোনো কারণে ইউজার না থাকে
                await acur.execute("INSERT INTO users (user_id, first_name) VALUES (%s, %s)", (user_id, query.from_user.first_name))
                user_data = (user_id, query.from_user.first_name, 0, False, None, None) # একটি টেম্পোরারি টাপল
